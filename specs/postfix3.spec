...
# Allow for an unpatched build for debugging purposes.
...
%define postfix_lib_dir     %{_prefix}/lib/postfix
%define postfix_dynamicmaps_dir %{postfix_config_dir}/dynamicmaps.cf.d
...
Source0:                ftp://ftp.porcupine.org/mirrors/postfix-release/official/postfix-%{version}%{?relc}.tar.gz
...
# 200 and up are dynamic map config files.
Source200:              dynamicmaps.cf
Source201:              ldap.cf
Source202:              mysql.cf
Source203:              pcre.cf
Source204:              pgsql.cf
...
Patch3:                 postfix-2.11.3-alternatives.patch
Patch8:                 postfix-3.0.0-large-fs.patch
Patch10:                postfix-3.1.3-posttls-finger.patch

# pflogsumm patches

Patch100:               pflogsumm-1.1.5-datecalc.patch
...
%{?with_eai:BuildRequires: libicu-devel}
...
%package ldap
Summary:                LDAP Database Support for Postfix
...
%description ldap
Install this package if you need support for the LDAP database type in postfix
%endif
...
%package lmdb
Summary:                LMDBtabase Support for Postfix
Group:                  Applications/System
Requires:               %{name} = %{epoch}:%{version}-%{release}
BuildRequires:          lmdb-devel
Provides:               postfix-lmdb

%description lmdb
Install this package if you need support for the LMDB database type in postfix
%endif
...
%package pgsql
Summary:                PostgreSQL Database Support for Postfix
Group:                  Applications/System
Requires:               %{name} = %{epoch}:%{version}-%{release}
...
%description pgsql
Install this package if you need support for the PostgreSQL database type in postfix
%endif
...
%package sqlite
Summary:                SQLite Database Support for Postfix
Group:                  Applications/System
Requires:               %{name} = %{epoch}:%{version}-%{release}
...
%description sqlite
Install this package if you need support for the SQLite database type in postfix
%endif
...
%package cdb
Summary:                CDB Database Support for Postfix
Group:                  Applications/System
Requires:               %{name} = %{epoch}:%{version}-%{release}
BuildRequires:          tinycdb-devel
Provides:               postfix-cdb

%description cdb
Install this package if you need support for the CDB database type in postfix
%endif
...
%prep
%setup -q -n postfix-%{version}%{?relc}
...
%patch10 -p1 -b .posttls-finger
%{!?with_eai: printf "smtputf8_enable = no\n" >> conf/main.cf}
%endif
...
%if %{with ldap}
  ...
  %global auxlibs_x %{?auxlibs_x} AUXLIBS_LDAP="-lldap -llber"
%endif
%if %{with lmdb}
  CCARGS="${CCARGS} -DHAS_LMDB"
  %global auxlibs_x %{?auxlibs_x} AUXLIBS_LMDB=-llmdb
%endif
%if %{with pcre}
  # -I option required for pcre 3.4 (and later?)
  CCARGS="${CCARGS} -DHAS_PCRE -I%{_includedir}/pcre"
  %global auxlibs_x %{?auxlibs_x} AUXLIBS_PCRE=-lpcre
%endif
%if %{with mysql}
  CCARGS="${CCARGS} -DHAS_MYSQL -I%{_includedir}/mysql"
  %global auxlibs_x %{?auxlibs_x} AUXLIBS_MYSQL="-L%{_libdir}/mysql -lmysqlclient -lm"
%endif
%if %{with pgsql}
  CCARGS="${CCARGS} -DHAS_PGSQL -I%{_includedir}/pgsql"
  %global auxlibs_x %{?auxlibs_x} AUXLIBS_PGSQL=-lpq
%endif
%if %{with sqlite}
  CCARGS="${CCARGS} -DHAS_SQLITE"
  %global auxlibs_x %{?auxlibs_x} AUXLIBS_SQLITE="-lsqlite3 -lpthread"
%endif
%if %{with cdb}
  CCARGS="${CCARGS} -DHAS_CDB"
  %global auxlibs_x %{?auxlibs_x} AUXLIBS_CDB=-lcdb
%endif
%if %{with sasl}
...
AUXLIBS="${AUXLIBS} -Wl,-z,relro"

make -f Makefile.init makefiles pie=yes shared=yes dynamicmaps=yes \
     CCARGS="${CCARGS}" AUXLIBS="${AUXLIBS}" %{?auxlibs_x} DEBUG="" \
...
%if %{with patches}
for x in mailq newaliases sendmail; do
    mv man/man1/$x.1 man/man1/$x.postfix.1
done
...
# Install dynamic map files
%{__install} -d $RPM_BUILD_ROOT%{postfix_dynamicmaps_dir}
%{__install} %{SOURCE200} $RPM_BUILD_ROOT%{postfix_config_dir}
%{__install} %{SOURCE201} %{SOURCE202} %{SOURCE203} %{SOURCE204} %{SOURCE205} \
         %{SOURCE206} %{SOURCE207} \
         $RPM_BUILD_ROOT%{postfix_dynamicmaps_dir}
...
# install collate
install -c auxiliary/collate/collate.pl $RPM_BUILD_ROOT%{postfix_command_dir}/collate
install -c auxiliary/collate/README $RPM_BUILD_ROOT%{postfix_readme_dir}/COLLATE_README
...
%post
%{?with_sysvinit:/sbin/chkconfig --add postfix}
%{?with_systemd:%systemd_post postfix.service}
...
%preun
%{?with_systemd:%systemd_preun postfix.service}
...
%postun
%{?with_systemd:%systemd_postun_with_restart postfix.service}
...
%{?with_sysvinit:%attr(0755, root, root) %{_initrddir}/postfix}
%{?with_systemd:%{_unitdir}/postfix.service}
%exclude %{postfix_dynamicmaps_dir}/*.cf
...
%endif
%exclude %{postfix_readme_dir}/LDAP_README
%exclude %{postfix_readme_dir}/LMDB_README
%exclude %{postfix_readme_dir}/MYSQL_README
%exclude %{postfix_readme_dir}/PCRE_README
%exclude %{postfix_readme_dir}/PGSQL_README
%exclude %{postfix_readme_dir}/SQLITE_README
%exclude %{postfix_readme_dir}/CDB_README
%exclude %{postfix_readme_dir}/COLLATE_README

# Misc files

%dir %attr(0755, root, root) %{postfix_config_dir}
%dir %attr(0755, root, root) %{postfix_dynamicmaps_dir}
...
%{?with_patches:%attr(0644, root, root) %{_mandir}/man1/*.postfix.1*}
...
#%exclude %{_mandir}/man5/cdb_table.5*
%{?with_patches:%attr(0644, root, root) %{_mandir}/man5/*.postfix.5*}
...
%attr(0644, root, root) %config(noreplace) %{postfix_config_dir}/dynamicmaps.cf
...
%attr(0755, root, root) %{postfix_lib_dir}/libpostfix-*.so
%if %{with patches}
%attr(0755, root, root) %{_prefix}/lib/sendmail.postfix
...
%if ! %{with patches}
%config(noreplace) %{postfix_config_dir}/*.default
%config(noreplace) %{postfix_config_dir}/makedefs.out
...
%files ldap
%defattr(-, root, root)
%attr(0755, root, root) %{postfix_lib_dir}/postfix-ldap.so
%attr(0644, root, root) %config(noreplace) %{postfix_dynamicmaps_dir}/ldap.cf
%doc %attr(0644, root, root) %{postfix_readme_dir}/LDAP_README
%doc %attr(0644, root, root) %{_mandir}/man5/ldap_table.5*
%endif

%if %{with lmdb}
%files lmdb
%defattr(-, root, root)
%attr(0755, root, root) %{postfix_lib_dir}/postfix-lmdb.so
%attr(0644, root, root) %config(noreplace) %{postfix_dynamicmaps_dir}/lmdb.cf
%doc %attr(0644, root, root) %{postfix_readme_dir}/LMDB_README
%doc %attr(0644, root, root) %{_mandir}/man5/lmdb_table.5*
%endif

%if %{with mysql}
%files mysql
%defattr(-, root, root)
%attr(0755, root, root) %{postfix_lib_dir}/postfix-mysql.so
%attr(0644, root, root) %config(noreplace) %{postfix_dynamicmaps_dir}/mysql.cf
%doc %attr(0644, root, root) %{postfix_readme_dir}/MYSQL_README
%doc %attr(0644, root, root) %{_mandir}/man5/mysql_table.5*
%endif

%if %{with pcre}
%files pcre
%defattr(-, root, root)
%attr(0755, root, root) %{postfix_lib_dir}/postfix-pcre.so
%attr(0644, root, root) %config(noreplace) %{postfix_dynamicmaps_dir}/pcre.cf
%doc %attr(0644, root, root) %{postfix_readme_dir}/PCRE_README
%doc %attr(0644, root, root) %{_mandir}/man5/pcre_table.5*
%endif

%if %{with pgsql}
%files pgsql
%defattr(-, root, root)
%attr(0755, root, root) %{postfix_lib_dir}/postfix-pgsql.so
%attr(0644, root, root) %config(noreplace) %{postfix_dynamicmaps_dir}/pgsql.cf
%doc %attr(0644, root, root) %{postfix_readme_dir}/PGSQL_README
%doc %attr(0644, root, root) %{_mandir}/man5/pgsql_table.5*
%endif

%if %{with sqlite}
%files sqlite
%defattr(-, root, root)
%attr(0755, root, root) %{postfix_lib_dir}/postfix-sqlite.so
%attr(0644, root, root) %config(noreplace) %{postfix_dynamicmaps_dir}/sqlite.cf
%doc %attr(0644, root, root) %{postfix_readme_dir}/SQLITE_README
%doc %attr(0644, root, root) %{_mandir}/man5/sqlite_table.5*
%endif

%if %{with cdb}
%files cdb
%defattr(-, root, root)
%attr(0755, root, root) %{postfix_lib_dir}/postfix-cdb.so
%attr(0644, root, root) %config(noreplace) %{postfix_dynamicmaps_dir}/cdb.cf
%doc %attr(0644, root, root) %{postfix_readme_dir}/CDB_README
#%doc %attr(0644, root, root) %{_mandir}/man5/cdb_table.5*
%endif
...
%attr(0755, root, root) %{postfix_command_dir}/collate
%doc %attr(0644, root, root) %{postfix_readme_dir}/COLLATE_README
%if %{with pflogsumm}
...
%changelog
* Fri Jun 15 2018 Peter Ajamian <pj@ghettoforge.org> - 2:3.3.1-1
- 3.3.1

* Mon Mar 05 2018 Peter Ajamian <pj@ghettoforge.org> - 2:3.3.0-1
- 3.3.0

* Mon Jan 29 2018 Peter Ajamian <pj@ghettoforge.org> - 2:3.2.5-1
- 3.2.5

* Mon Oct 30 2017 Peter Ajamian <pj@ghettoforge.org> - 2:3.2.4-1
- 3.2.4
- Remove sendmail patch.

* Wed Oct 25 2017 Peter Ajamian <pj@ghettoforge.org> - 2:3.2.3-2
- Fix dynamic maps support in sendmail binary.

* Tue Sep 26 2017 Peter Ajamian <pj@ghettoforge.org> - 2:3.2.3-1
- 3.2.3
- Remove propagate_unmatched_extensions patch.

* Mon Jul 24 2017 Peter Ajamian <pj@ghettoforge.org> - 2:3.2.2-4
- move alternatives --install to posttrans script to fix problem with
  alternatives getting wiped on upgrade from stock postfix.

* Sat Jul 08 2017 Peter Ajamian <pj@ghettoforge.org> - 2:3.2.2-3
- add collate command to -perl-scripts.

* Thu Jun 22 2017 Peter Ajamian <pj@ghettoforge.org> - 2:3.2.2-2
- Patch for propagate_unmatched_extensions regression.

* Thu Jun 15 2017 Peter Ajamian <pj@ghettoforge.org> - 2:3.2.2-1
- 3.2.2
- ./DB_CONFIG patch no longer needed.

* Mon Jun 12 2017 Peter Ajamian <pj@ghettoforge.org> - 2:3.2.1-1
- 3.2.1
- Fix 20170611 workaround for undocumented ./DB_CONFIG read.

* Mon Mar 06 2017 Peter Ajamian <pj@ghettoforge.org> - 2:3.2.0-1
- 3.2.0
- Pass -keep-build-mtime flag to postfix-install as per
  https://bugzilla.redhat.com/show_bug.cgi?id=1307064 and
  http://postfix.1071664.n5.nabble.com/PATCH-Preserve-timestamps-during-make-install-td85492.html

* Sat Jan 07 2017 Peter Ajamian <pj@ghettoforge.org> - 2:3.1.4-2
- pflogsumm to 1.1.5
- Rebase pflogsumm datecalc patch.

* Wed Jan 04 2017 Peter Ajamian <pj@ghettoforge.org> - 2:3.1.4-1
- 3.1.4
- Removed postscreen race patch, now in upstream.

* Tue Jan 03 2017 Peter Ajamian <pj@ghettoforge.org> - 2:3.1.3-3
- Fix dynamic mapping postmap support for cdb, lmdb and sdbm database types.
- Added the posttls-finger utility.

* Wed Oct 19 2016 Peter Ajamian <pj@ghettoforge.org> - 2:3.1.3-2
- Build with postscreen cache race patch.

* Sun Oct 16 2016 Peter Ajamian <pj@ghettoforge.org> - 2:3.1.3-1
- 3.1.3
- Build without postscreen cache race patch.

* Fri Sep 02 2016 Peter Ajamian <pj@ghettoforge.org> - 2:3.1.2-4
- Build with postscreen cache race patch.

* Fri Sep 02 2016 Peter Ajamian <pj@ghettoforge.org> - 2:3.1.2-3
- Remove conflicts for postfix-perl-scripts, it was preventing installation.
- Build without postscreen cache race patch.

* Wed Aug 31 2016 Peter Ajamian <pj@ghettoforge.org> - 2:3.1.2-2
- Patch for postscreen cache race condition bug.

* Wed Aug 31 2016 Peter Ajamian <pj@ghettoforge.org> - 2:3.1.2-1
- 3.1.2

* Tue Jul 19 2016 Peter Ajamian <pj@ghettoforge.org> - 2:3.1.1-3
- Remove conflicts, they were causing problems with upgrades.

* Tue Jul 19 2016 Peter Ajamian <pj@ghettoforge.org> - 2:3.1.1-2
- Add missing provides and conflicts.

* Mon May 16 2016 Peter Ajamian <pj@ghettoforge.org> - 2:3.1.1-1
- 3.1.1

* Sat Feb 27 2016 Peter Ajamian <pj@ghettoforge.org> - 2:3.1.0-1
- 3.1.0
- New file: /usr/libexec/postfix/postfix-tls-script

* Mon Oct 12 2015 Peter Ajamian <pj@ghettoforge.org> - 2:3.0.3-1
- 3.0.3
- Reset-errno patch no longer needed

* Fri Jul 24 2015 Peter Ajamian <pj@ghettoforge.org> - 2:3.0.2-1
- 3.0.2

* Thu Apr 23 2015 Peter Ajamian <pj@ghettoforge.org> - 2:3.0.1-2
- Fix incorrect warning about missing dynamicmaps.cf.d

* Tue Apr 14 2015 Peter Ajamian <pj@ghettoforge.org> - 2:3.0.1-1
- 3.0.1

* Mon Feb 23 2015 Peter Ajamian <pj@ghettoforge.org> - 2:3.0.0-3
- We were still building the RC, bleh
- Remove pie patch
- EAI support only for el7

* Mon Feb 23 2015 Peter Ajamian <pj@ghettoforge.org> - 2:3.0.0-2
- Add EAI support
- Change default inet_protocols in config

* Mon Feb 09 2015 Peter Ajamian <pj@ghettoforge.org> - 2:3.0.0-1
- 3.0.0 stable

* Fri Feb 06 2015 Peter Ajamian <pj@ghettoforge.org> - 2:3.0.0-0.rc1.4
- Change to newer patch for pie support
- stack protection
- Added CDB support

* Thu Feb 05 2015 Peter Ajamian <pj@ghettoforge.org> - 2:3.0.0-0.rc1.3
- Build for el7
- systemd support for el7

* Wed Feb 04 2015 Peter Ajamian <pj@ghettoforge.org> - 2:3.0.0-0.rc1.2
- Build for el6
- Update largefs patch
- Fix lmdb support

* Tue Feb 03 2015 Peter Ajamian <pj@ghettoforge.org> - 2:3.0.0-0.rc1.1
- Initial build of Postfix 3.0 RC1
- Added pflogsumm
- Dynamic database module support
- use make non-interactive-package for install instead of sh post-install
- Split into individual database packages
- Patch to fix -pie support

* Fri Nov 21 2014 Peter Ajamian <peter@pajamian.dhs.org> - 2:2.11.3-1
- Bump to 2.11.3
- Added sqlite support
- Added lmdb support

* Fri Jan 17 2014 Peter Ajamian <peter@pajamian.dhs.org> - 2:2.10.3-1
- Update to 2.10.3

* Sun Jan  5 2014 Peter Ajamian <peter@pajamian.dhs.org> - 2:2.10.2-3
- Rebuild to pick up EC ciphers in el6.5

* Wed Nov  6 2013 Peter Ajamian <peter@pajamian.dhs.org> - 2:2.10.2-2
- Enable MySQL, Postgresql and LDAP support

* Sun Nov  3 2013 Bryan Seitz <seitz@ghettoforge.org> - 2:2.10.2-1
